...
public class ContractServiceImplementationTest extends AbstractJUnit4SpringContextTests {
   ...
   /* private static final String CREATE_SCRIPT = "mobile/src/main/resources/sql/create-data-contract.sql";
    ...
    private static final int CONTRACT_ID = 213698745;
    private static final int TARIFF_ID = 211369878;
    private static final long CONTRACT_NUMBER =  2030508090;
    ...
    public void testContractRead() {
        ...
        assertTrue(contract.getNumber() == CONTRACT_NUMBER);
    }
    ...
    public void testContractUpdate() {
        ...
        contract.setTariff(tariffService.getEntityById(TARIFF_ID));
        contract.addOption(optionService.getEntityById(214561783));
        contractService.updateEntity(contract);
        contract = contractService.getEntityById(contract.getId());
        assertTrue(contract.getTariff().getId() == TARIFF_ID);
        assertFalse(contract.getOptions().isEmpty());
        assertTrue(optionService.getAllOptionsForContract(CONTRACT_ID).size() == 1);
    ...
    public void testContractDelete() {
        Contract contract = contractService.getEntityById(CONTRACT_ID);
        contractService.deleteEntity(contract);
        assertNull(contractService.getEntityById(CONTRACT_ID));
    }
    ...
    public void testContractGetByNumberSuccess() {
        Contract contract = contractService.getContractByNumber(CONTRACT_NUMBER);
